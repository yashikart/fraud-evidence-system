...
export default function Timeline({ reports, events }) {
  const [sortAsc, setSortAsc] = useState(true);

  // Determine which data to use - events or reports
  const dataToUse = events || reports || [];
  
  // Ensure dataToUse is an array
  const dataArray = Array.isArray(dataToUse) ? dataToUse : [];
  
  // ✅ Sort reports by date
  const sorted = [...dataArray].sort((a, b) => {
    // Handle different data structures
    const dateA = new Date(a.date || a.timestamp || "1970-01-01");
    const dateB = new Date(b.date || b.timestamp || "1970-01-01");
  ...
  const grouped = sorted.reduce((acc, rpt) => {
    // Handle different data structures
    const dateKey = rpt.date || rpt.timestamp || "Unknown Date";
    const date = new Date(dateKey).toLocaleDateString();
    (acc[date] = acc[date] || []).push(rpt);
    return acc;
  }, {});
              ...
              {grouped[date].map((report, index) => (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between w-full">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        {report.wallet ? (
                          ...
                          <>
                            ID: {report._id ? report._id.slice(0, 8) : 'N/A'}
                          </>
                        )}
                      </p>

                      {/* ✅ Risk or event details */}
                    ...
                    {/* ✅ Risk Badge or event type badge */}
                    <div
